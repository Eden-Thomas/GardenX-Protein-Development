...
class NeurosnapClient:
    ...
    async def predict_thermostability_temstapro(self, sequence: str, temperature: int = 50, job_name: str = "D1_thermostability") -> Dict:
        ...
        try:
            async with self.session.post(f"{self.base_url}/temstapro/submit", headers=self.headers, json=payload) as response:
    ...
    async def batch_predict_temstapro(self, sequences: List[str], temperature: int = 50) -> List[Dict]:
        """Batch prediction for multiple sequences"""
        tasks = []
        for i, seq in enumerate(sequences):
            task = self.predict_thermostability_temstapro(seq, temperature, job_name=f"D1_variant_{i}")
            tasks.append(task)
        return await asyncio.gather(*tasks)
    
    async def predict_mutation_effects_thermompnn(self, structure_pdb: str, mutations: List[Dict], job_name: str = "D1_mutations") -> List[Dict]:
        """Predict stability changes for mutations using ThermoMPNN"""
        if not self.session:
            self.session = aiohttp.ClientSession()
        
        formatted_mutations = [f"{m['from']}{m['position']}{m['to']}" for m in mutations]
        ...
        try:
            async with self.session.post(f"{self.base_url}/thermompnn/submit", headers=self.headers, json=payload) as response:
                response.raise_for_status()
                result = await response.json()
                job_id = result.get('job_id')
    ...
    async def predict_structure_alphafold(self, sequence: str, job_name: str = "D1_structure", use_templates: bool = False) -> Dict:
        ...
        try:
            async with self.session.post(f"{self.base_url}/alphafold2/submit", headers=self.headers, json=payload) as response:
                response.raise_for_status()
                result = await response.json()
                job_id = result.get('job_id')
    ...
    async def generate_variants_rfdiffusion(self, template_pdb: str, design_regions: List[tuple], num_designs: int = 50, job_name: str = "D1_design") -> List[Dict]:
        ...
        try:
            async with self.session.post(f"{self.base_url}/rfdiffusion/submit", headers=self.headers, json=payload) as response:
                response.raise_for_status()
                result = await response.json()
                job_id = result.get('job_id')
    ...
    async def _wait_for_job(self, job_id: str, tool_name: str, max_wait_seconds: int = 1800, poll_interval: int = 10) -> Dict:
        ...
        while elapsed < max_wait_seconds:
            try:
                async with self.session.get(f"{self.base_url}/{tool_name}/status/{job_id}", headers=self.headers) as response:
                    response.raise_for_status()
    ...
    async def get_account_info(self) -> Dict:
        ...
        try:
            async with self.session.get(f"{self.base_url}/account", headers=self.headers) as response:
                response.raise_for_status()
    ...
    async def list_jobs(self, limit: int = 10) -> List[Dict]:
        ...
        try:
            async with self.session.get(f"{self.base_url}/jobs?limit={limit}", headers=self.headers) as response:
                response.raise_for_status()
                return await response.json()
        except aiohttp.ClientError as e:
...
async def quick_thermostability_check(sequence: str, target_temp: int = 50, api_key: Optional[str] = None) -> Dict:
    """Quick function to check thermostability"""
    if api_key is None:
...
async def quick_structure_prediction(sequence: str, api_key: Optional[str] = None) -> Dict:
